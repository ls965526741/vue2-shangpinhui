    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          ...
          <Zoom :img="zoomImg" />
          <!-- 小图列表 -->
          <ImageList @imgChange="imgChange" :list="skuInfo.skuImageList" />
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="item in spuSaleAttrList" :key="item.id">
                ...
                <dd
                  changepirce="90"
                  :class="item1.isActive === '1' ? 'active1' : item1.isChecked === '0' ? 'active' : ''"
                  v-for="(item1, index) in item.spuSaleAttrValueList"
                  :key="index"
                  @click="clickDetail(item.spuSaleAttrValueList, item1)"
                >
            ...
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" @input="inputChange" class="itxt" v-model="count" />
                <span @click="count += 1" class="plus">+</span>
                <span @click="count = count <= 0 ? 0 : count - 1" class="mins">-</span>
              </div>
              <div class="add">
...
export default {
  ...
  methods: {
    ...
    async add() {
      ...
      try {
        ...
        this.$router.push(`/addcart/${this.skuInfo.id}`)
      } catch (err) {
        console.log(err)
      }
    },
    inputChange(val) {
      if (!isNumber(val.target.value - 0)) {
        this.count = ''
      }
    },
    clickDetail(item, current) {
      item.map(item => (item.isActive = '0'))
      if (current.isChecked === '0') {
        current.isActive = '1'
      }
    }
  },
  ...
  computed: {
    ...mapState({ list: state => state.detail.detailList }),
    ...mapGetters(['categoryView', 'skuInfo', 'spuSaleAttrList']),
    zoomImg() {
      return this.defaultImg || this.skuInfo.skuDefaultImg
    }
  }
}
</script>
